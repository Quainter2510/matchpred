...
│   │                              # PlayerSearch, CountrySelect, Flag, TeamName, …
│   ├── api/
│   │   ├── client.ts              # fetch + JWT + auto-refresh
│   │   └── endpoints.ts           # типизированные вызовы API
│   ├── store/auth.ts              # Zustand: user, loading, loadMe
│   └── utils/                     # scoring.ts (превью очков), dates.ts, stage.ts, countries.ts
└── vite.config.ts
```
