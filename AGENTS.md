...
│       ├── top_scorers.py     # снимок бомбардиров турнира в Redis (суточный)
...
├── alembic/versions/          # 0001…0011 (rooms — 0004, team_matches — 0008, room_match_multipliers — 0009, winner_team — 0010, очистка осиротевших прогнозов — 0011)
...
| **Итоговый бомбардир** | ✓ (своя) | ✓ (своя комната) | — |
...
| POST | `/rooms/{id}/special-prediction/scorer-result` | RAdmin | `{player_api_id, player_name}` → начислить очки за бомбардира **только в этой комнате** |
...
| GET | `/rooms/{id}/standings/top-scorers` | Member | `{updated_at, top: [{name, photo, team, goals}]×5, predicted: [{name, photo, goals, backers}]}`. Топ-5 бомбардиров турнира + игроки из спецпрогнозов **этой комнаты** (только текущих участников; с голами и числом выбравших). ID канонизируется (реальный API-ID и кураторский = один игрок), имена — русские из каталога. Из суточного снимка в Redis (`top_scorers:snapshot`), читатель к API не ходит. Обновляется кроном в 10:00 МСК и при `/admin/sync` |
...
- Начисление за чемпиона — автоматически при пересчёте после завершения финала (`stage='final'`) во всех активных комнатах, **при любом исходе**. Победитель = `final.winner_team` (пенальти/допвремя), иначе по счёту ОВ. При синхронизации с API-Football победитель проставляется сам; для ручного ввода ничьей в финале суперадмин указывает `winner_team`.
- Начисление за бомбардира — **вручную и по комнатам**: `POST /rooms/{id}/special-prediction/scorer-result` (RAdmin). Каждая комната определяет бомбардира самостоятельно; начисляет только в своей.
- Спецпрогнозы других скрыты до дедлайна (раскрывает раньше только суперадмин); в таблице лидеров до старта видны только флажки «выбран/не выбран».
...
9. **Результат матча глобален и вводится только суперадмином** — один счёт затрагивает все активные комнаты (каждая по своему коэффициенту); правки кода пересчёта обязаны сохранять идемпотентность. **Коэффициент матча и итоговый бомбардир — по комнатам** (`room_match_multipliers`, `/scorer-result`), ими управляет админ комнаты только в своей комнате.
