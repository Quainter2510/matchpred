...
│       ├── players_ru.py      # словарь латиница→RU имён игроков РПЛ (бомбардир)
...
| GET | `/rooms/{id}/special-prediction/scorer-search?q=` | Member | Поиск бомбардира с учётом турнира: лиги — только клубы этой лиги + рус. имена; ЧМ — глобально |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|---------|
| POST | `/admin/sync` | SA | Синхронизация всех активных лиг + логотипы + пересчёт + снимки бомбардиров. Устойчиво к сбоям: каждая лига в SAVEPOINT (`sync.fetch_and_apply_all`), ошибка лиги → в `errors[]`, не роняет остальные |
...
**Подпись тура:** для лиг тур подписывается номером «N-й тур» (`tours.round_label` из `matches.round`, напр. «Regular Season - 5»), а не датой. Бэкенд отдаёт её в `MatchDay.label` (если у всех матчей дня один номер), фронт/бот показывают номер, иначе — дату. Плей-офф/группы номера не имеют → показывается дата.
...
Бомбардиры считаются по лиге+сезону: снимок в Redis `top_scorers:snapshot:{league}:{season}` (`top_scorers.refresh_all_top_scorers` — по лигам активных турниров со спецпрогнозом бомбардира). **Поиск бомбардира тип-зависим** (`GET /rooms/{id}/special-prediction/scorer-search`): для лиговых турниров — снимок бомбардиров лиги + курируемый список `players_ru.py` (реальный id резолвится через `/players/profiles` — работает и до старта сезона, когда составы ещё не загружены), имена по-русски; для ЧМ — глобальный поиск с кураторским списком сборников (`/players/search`). Спецпрогнозы других скрыты до срока (раскрывает раньше только суперадмин).
...
*Prediction Site · AGENTS.md · v6.2 (поиск бомбардира по лиге + players_ru, туры по номеру, устойчивый мультилиговый синк; июль 2026)*
