...
| POST | `/matches/{mid}/result` | SA | `{home_score_ft, away_score_ft}` — только основное время. Начисляет очки во всех комнатах (каждая по своему коэффициенту); изменение уже введённого счёта снимает старые очки и начисляет заново (`rescore_match`) |
...
- Начисление за чемпиона — автоматически при пересчёте после ввода счёта финала (`stage='final'`, при ничьей не начисляется) во всех активных комнатах.
