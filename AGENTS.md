...
Платформа для групп участников (20–100 человек в комнате) — делать прогнозы на матчи ЧМ-2026, получать реальные результаты через API-Football и формировать турнирные таблицы.

**Архитектура: комнаты.** Матчи и их результаты — глобальные (общие для всей инсталляции). Поверх них параллельно идёт любое число **комнат** (соревнований): у каждой свой пароль, свои участники, свои правила начисления очков, свои прогнозы, спецпрогнозы и таблица лидеров. Один пользователь может состоять в нескольких комнатах одновременно.
...
│   │                          # require_any_admin / require_room_member / require_room_admin
...
│   │   ├── match.py           # Match (глобальные, с points_multiplier)
...
│       ├── scheduler.py       # APScheduler: тик каждые 5 мин + ежедневный полный синк
│       ├── audit.py           # запись в audit_log
│       ├── players_catalog.py # каталог игроков для автодополнения
...
│   │   └── admin/
│   │       ├── Admin.tsx          # глобальная панель (матчи, результаты, журнал)
│   │       ├── AdminMatches.tsx   # результаты, коэффициенты, синхронизация
│   │       ├── AdminSpecial.tsx   # начисление за бомбардира
│   │       ├── AdminSettings.tsx
...
| points_champion / points_scorer | SMALLINT | очки за чемпиона / бомбардира (по умолч. 10/10) |
...
| points_multiplier | SMALLINT DEFAULT 1 | бонусный коэффициент 0/1/2/3; 0 — аннулирование |
...
`champion_selected`, `top_scorer_selected`, `room_created`, `room_deleted`,
`room_joined`, `room_password_changed`, `room_rules_changed`, `api_sync`, `nickname_changed`
...
**Суперадмин** (`system_role`) — единственный в системе, назначается атомарно первому вошедшему. Создаёт/удаляет/архивирует комнаты, меняет их правила очков, входит в любую комнату без пароля (как админ), видит журнал, передаёт роль.

**Админ комнаты** (`room_role='admin'`) — управляет участниками/паролем/регламентом своей комнаты. Любой админ хотя бы одной комнаты (`require_any_admin`) получает доступ к **глобальным** операциям с матчами: результаты, коэффициенты, синхронизация, пересчёт, начисление за бомбардира — потому что результаты общие для всех комнат.
...
| Матчи: создать / редактировать / результат / коэффициент | ✓ | ✓ (глобально) | — |
| Синхронизация / пересчёт / бомбардир | ✓ | ✓ (глобально) | — |
| Передать роль суперадмина | ✓ | — | — |
| Журнал (audit_log) + CSV-экспорт | ✓ | — | — |
| Режим симуляции (X-Sim-Now) | ✓ | — | — |
| Режим обычного пользователя (X-View-As) | ✓ | — | — |
| Прогноз на матч (до kickoff) | ✓ | ✓ | ✓ |
| Спецпрогноз (до дедлайна комнаты) | ✓ | ✓ | ✓ |
| Видеть прогнозы других на матч | всегда | всегда (своя комната) | после kickoff |
| Видеть спецпрогнозы других | всегда | всегда (своя) | после дедлайна |
...
**RAdmin** — админ комнаты или суперадмин; **AnyAdmin** — админ любой комнаты или суперадмин;
**SA** — только суперадмин.
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|---------|
| GET | `/rooms?q=` | Pub | Лобби: активные комнаты, поиск по имени. Вошедшим — `is_member`/`my_role` |
| GET | `/rooms/my` | Auth | Мои комнаты (включая архивные) |
| POST | `/rooms` | SA | Создать комнату `{name, password, first_match_at?}`; создатель — первый админ |
...
| DELETE | `/rooms/{id}/members/{uid}` | RAdmin | Удалить из комнаты (не из системы) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|---------|
| GET | `/matches?date=` | AnyAdmin | Глобальный список для панели |
| POST | `/matches` | AnyAdmin | Создать матч вручную |
| PATCH | `/matches/{mid}` | AnyAdmin | Редактировать время/команды/стадию |
| PATCH | `/matches/{mid}/multiplier` | AnyAdmin | `{multiplier: 0│1│2│3}`; завершённый матч авто-пересчитывается |
| PATCH | `/matches/tour/{date}/multiplier` | AnyAdmin | Коэффициент на весь тур (все матчи даты) |
| POST | `/matches/{mid}/result` | AnyAdmin | `{home_score_ft, away_score_ft}` — только основное время. Начисляет очки во всех комнатах; изменение уже введённого счёта снимает старые очки и начисляет заново (`rescore_match`) |
...
| GET | `/rooms/{id}/special-prediction/all` | Member | Все спецпрогнозы комнаты (игрок — после дедлайна, RAdmin — всегда) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|---------|
| POST | `/admin/sync` | AnyAdmin | Синхронизация с API-Football + пересчёт |
| POST | `/admin/recalculate` | AnyAdmin | Пересчитать все незакрытые очки (идемпотентно) |
| POST | `/admin/scorer-result` | AnyAdmin | `{player_api_id, player_name}` → начислить очки за бомбардира во всех активных комнатах |
...
### Режим обычного пользователя (заголовок `X-View-As: player`)

Суперадмин может смотреть сайт глазами игрока: с заголовком
`X-View-As: player` room-контекст (`require_room_member`/`require_room_admin`)
считает его обычным участником — чужие прогнозы и спецпрогнозы скрыты до
дедлайна, `members_filled/members_total` в `/matches/days` равны null,
управление комнатой отвечает 403, в комнаты без членства не пускает. Для
не-суперадмина заголовок молча игнорируется. **Глобальная панель
(`require_any_admin`/`require_superadmin`) заголовок не учитывает** — иначе
режим нельзя было бы выключить. Переключатель — Глобальная панель →
чекбокс «Режим обычного пользователя» (фронт: `store/viewAs.ts`, индиговый
баннер `ViewAsBanner` с кнопкой возврата).
...
- Прогнозы других скрыты до `kickoff_at` (админ комнаты видит всегда).

### Спецпрогнозы (чемпион + бомбардир)

- Свои в каждой комнате; дедлайн — `room.first_match_at`.
- Начисление за чемпиона — автоматически при пересчёте после ввода счёта финала (`stage='final'`, при ничьей не начисляется).
- Начисление за бомбардира — **вручную**: `POST /admin/scorer-result`.
- Спецпрогнозы других скрыты до дедлайна; в таблице лидеров до старта видны только флажки «выбран/не выбран».
...
**Коэффициент матча** (`points_multiplier`, 0/1/2/3): очки за матч умножаются на него. Задаётся на матч или на весь тур. **×0 — аннулирование**: очки 0, и точный счёт не идёт в тайбрейк. Изменение коэффициента у завершённого матча автоматически пересчитывает уже начисленные очки.
...
- `score_match` — начисляет по прогнозам с `points_awarded IS NULL` (идемпотентно). Только для `status='finished'` — у live-матчей в тех же колонках текущий счёт.
- `rescore_match` — при исправлении уже введённого счёта или смене коэффициента: снимает прежние очки, затем начисляет заново.
...
/admin/*                     — глобальная панель (AnyAdmin): матчи, результаты,
                               коэффициенты, синхронизация, журнал (SA)
...
- APScheduler: тик каждые 5 мин, но внутри — дешёвая проверка по БД (`_should_poll`): опрос API-Football только если сегодня есть матч, есть live-матч или недавно начавшийся. Ежедневный полный синк в 03:00 UTC (с подтяжкой букв групп из /standings).
...
2. **Доп. время и пенальти не учитываются нигде** — ни в хранении, ни в очках.
...
6. **Чужие прогнозы — строго после kickoff**, спецпрогнозы — после `first_match_at` (админ комнаты — всегда).
...
9. **Результат матча глобален** — ввод одного счёта затрагивает все активные комнаты; правки кода пересчёта обязаны сохранять идемпотентность.
...
*ЧМ-2026 Prediction Site · AGENTS.md · v5.0 (актуализировано по коду, июнь 2026)*
