...
│   ├── store/                     # auth.ts (user, loadMe), sim.ts (режим симуляции)
...
Правила оверлея:
- матч, начавшийся к sim-моменту: реальный финальный счёт остаётся; live-счёт
  принимается как финальный; матч без счёта получает **детерминированный
  фейковый** (из UUID матча, стабилен между запросами), статус `finished`;
- очки пересчитываются в памяти теми же чистыми функциями `scoring.py`
